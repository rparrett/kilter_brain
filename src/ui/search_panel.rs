...
fn update_search_field(
    ...
    if keyboard_input.pressed(KeyCode::Backspace) {
      ...
      } else if keyboard_input.just_pressed(KeyCode::Backspace) || time.elapsed_seconds().fract() < 0.05 {
...
fn update_search_results(
  ...
  let Ok(search_text) = search_field.get_single().map(|text| &text.sections[0].value) else {
    return;
  };
    ...
    for (interaction, children) in query.iter() {
        if *interaction == Interaction::Pressed {
            if let Ok(text) = text_query.get(children[0]) {
                ...
                let climb_id = text.sections[0].value.split(":").next().unwrap().parse().unwrap();
